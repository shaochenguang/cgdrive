...
CGDShellMalloc::operator LPMALLOC() const
{
    return m_pMalloc;
}

LPVOID CGDShellMalloc::Alloc(ULONG cb)
{
    return m_pMalloc->Alloc(cb);
}
void CGDShellMalloc::Free(LPVOID p)
{
    m_pMalloc->Free(p);
}

HRESULT CGDModule::Init(__in _ATL_OBJMAP_ENTRY* p, __in HINSTANCE h, 
    __in LPCWSTR lpszResRelativeDir, __in_opt const GUID* plibid /*= NULL*/)
{
    ...
    wcscpy_s(m_szDN_CGDROOT, sizeof(m_szDN_CGDROOT) / sizeof(WCHAR), CGD_NODE_NAME);
